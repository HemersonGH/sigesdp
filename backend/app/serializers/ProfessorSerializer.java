...
public class ProfessorSerializer {
	...
	static {
		...
		listAll = new JSONSerializer().include("id", "nome", "curriculoLates", "areaConhecimento.id",
				"areaConhecimento.nome", "formacaoAcademica", "sala", "telefone", "departamento.id",
				"departamento.nome", "departamento.sigla", "usuario.id", "usuario.email").exclude("*")
				.prettyPrint(prettyPrint);

		listDetails = new JSONSerializer()
